...
import { InvoiceAuditBrandInvoiceInterface } from "../../../interfaces/invoiceAuditInterface";

const InvoiceTableComponent: React.FC<commonProps<InvoiceAuditBrandInvoiceInterface[]>> = ({ data = [] }) => {
    ...
    const groupedData = data.reduce((acc, item) => {
        ...
        return acc;
    }, {} as Record<string, InvoiceAuditBrandInvoiceInterface[]>);
                ...
                <thead className="bg-gray-200 dark:bg-[#5e5d5d] text-sm font-medium top-0 z-10 dark:text-[#dad6d6]">
                    <tr>
                        ...
                        {[
                            ...
                            { icon: <FaBoxOpen />, label: "PullPO" },
                            { icon: <FaBoxOpen />, label: "UPC" },
                            { icon: <FaBoxOpen />, label: "SKU" },
                            { icon: <FaBoxOpen />, label: "ITEM COST" },
                            { icon: <FaBoxOpen />, label: "DISC. COST" },
                            { icon: <FaBoxOpen />, label: "INVQTY" },
                            { icon: <FaBoxOpen />, label: "TOTALINVCOST" },
                            { icon: <FaBoxOpen />, label: "INVvsDDDISCCOST" },
                            ...
                            {expandedGroups[poNumber] &&
                                groupedData[poNumber]?.map((item, index) => (
                                    ...
                                    >
                                        ...
                                        <td className="p-2 border-b text-left">{item.pullPo}</td>
                                        ...
                                        <td className="p-2 border-b text-left">{item.invQuantity}</td>
                                        <td className="p-2 border-b text-left">{item.totalInvCost}</td>
