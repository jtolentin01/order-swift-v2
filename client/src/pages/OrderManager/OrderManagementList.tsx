...
const OrderManagementPageComponent: React.FC<ComponentProps> = ({ isDisabled }) => {
    const dispatch = useAppDispatch();
    const { items: orderManagers, loading, error } = useAppSelector((state) => state.orderManager);
            ...
            <div className={`${disabled ? 'blur-sm' : ''} w-full h-full flex flex-col justify-between`}>
                <div>
                    <div className="flex justify-between items-center mb-4">
                        <h1 className="text-md font-bold text-red-600 mb-4">Order Management</h1>
                    ...
                    {orderManagers.length > 0 ? (
                        <div className={`grid gap-4 ${viewType === 'card' ? 'sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4' : 'grid-cols-1'}`}>
                            {orderManagers
                                .filter((om) => currentUser[0].orderManagerAccess.includes(om.orderManagerId))
                                .map((item) => (
                                    <NavLink key={item.orderManagerId} to={item.orderManagerId}>
                                        <div className={`flex ${viewType === 'card' ? 'flex-col hover:scale-105 h-64' : 'flex-row items-center h-24'} bg-white border border-gray-200 p-2 rounded-lg shadow-lg dark:bg-[#333333] dark:border-[#444444] hover:bg-[#F9F9F9] dark:hover:bg-[#444444]`}>
                                            ...
                                            <div className={`flex ${viewType === 'card' ? 'items-center justify-left mb-2' : 'items-center justify-center mr-4'}`}>
                                                <div className="w-16 h-16 rounded-full border p-1 border-[#E0E0E0]">
                                                    <img src={item.thumbnail} alt={`${item.orderManagerName} logo`} className="h-full w-full rounded-full object-cover" />
                                                </div>
                                            ...
                                            <div className="flex flex-col items-start flex-grow overflow-hidden">
                                                <h2 className="text-lg font-bold text-black dark:text-white mb-1">{item.orderManagerName}</h2>
                                                <p className="text-sm text-gray-500 dark:text-[#E0E0E0] overflow-hidden text-ellipsis">{item.description}</p>
                                            </div>
                                        </div>
                                    </NavLink>
                                ))}
                ...
                <div>
                    <p className="text-gray-600 dark:text-[#E0E0E0] mt-4">{orderManagers.length} Active brand(s) for Ordering</p>
                </div>
                {
...
export default OrderManagementPageComponent;